...
그대로 멈춘다 — 둘 중 하나가 밀린 상태로 덮어쓰는 것이 가장 나쁘기 때문이다.
"""
import io
import os
import re
...
def run(deck_path, md_path, check_only=False):
    ...
    if len(slides) != len(secs):
        sys.exit("슬라이드 %d 장, 대본 %d 장 — 수가 달라서 멈춤"
                 % (len(slides), len(secs)))
    ...
    for num, slide in enumerate(slides, 1):
        ...
        if [l.strip() for l in have.splitlines() if l.strip()] == \
                [l.strip() for l in want.splitlines() if l.strip()] and have == want:
            continue
