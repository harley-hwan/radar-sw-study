...
#define TGT_TWO_PI					(2.0 * PI)

// 참고 소스 코드의 변환 함수는 값만 돌려줄 뿐 실패를 알리지 못한다. 그래서 결과가 유한한지 여기서 직접 본다.
static INT32 f_Tgt_IsFiniteRect(const STRUCT_Coord_Rect *st_Vec)
{
	return (INT32)((isfinite(st_Vec->x) != 0) && (isfinite(st_Vec->y) != 0) && (isfinite(st_Vec->z) != 0));
}

// 위경고도가 모두 유한한지 본다.
static INT32 f_Tgt_IsFiniteLla(const STRUCT_Coord_Lla *st_Lla)
{
	return (INT32)((isfinite(st_Lla->Lat) != 0) && (isfinite(st_Lla->Lon) != 0) && (isfinite(st_Lla->Alt) != 0));
}

// 각을 -PI ~ PI 로 접는다. 참고 소스 코드에는 없어 여기에 둔다.
static DOUBLE64 f_Tgt_WrapAngle(DOUBLE64 angle)
{
	DOUBLE64 wrapped;

	wrapped = fmod(angle + PI, TGT_TWO_PI);
...
static EN_TgtStatus f_Tgt_GetAttRate(STRUCT_Coord_Attitude *st_Rate, const ST_TargetInit *st_Target, FLOAT64 simTime)
{
	EN_TgtStatus			enStatus;
...
EN_TgtStatus f_Tgt_InitState(ST_TargetState *st_State, const STRUCT_Coord_Lla *st_InitLla, const STRUCT_Coord_Attitude *st_InitAtt, FLOAT64 headingSpeed)
{
	EN_TgtStatus	enStatus;
	...
	else
	{
		...
		st_New.st_Att.Roll	= f_Tgt_WrapAngle(st_InitAtt->Roll);
		st_New.st_Att.Pitch	= f_Tgt_WrapAngle(st_InitAtt->Pitch);
		st_New.st_Att.Yaw	= f_Tgt_WrapAngle(st_InitAtt->Yaw);

		// 참고 소스의 f_Trans_Lla_To_Ecef 는 경도를 먼저 받는다.
		st_New.st_PosEcef	= f_Trans_Lla_To_Ecef(st_New.st_Lla.Lon, st_New.st_Lla.Lat, st_New.st_Lla.Alt);
		st_New.st_VelEcef	= f_Tgt_GetVelEcef(&st_New.st_Att, &st_New.st_Lla, headingSpeed);

		if ((f_Tgt_IsFiniteRect(&st_New.st_PosEcef) != 0) && (f_Tgt_IsFiniteRect(&st_New.st_VelEcef) != 0))
		{
			*st_State	= st_New;
			enStatus	= TGT_OK;
		}
...
static EN_TgtStatus f_Tgt_Propagate(ST_TargetState *st_State, const STRUCT_Coord_Attitude *st_Rate, FLOAT64 headingSpeed, FLOAT64 stepTime, FLOAT64 nextTime)
{
	EN_TgtStatus			enStatus;
	...
	st_Next.st_Att.Roll		= f_Tgt_WrapAngle(st_State->st_Att.Roll + (st_Rate->Roll * stepTime));
	st_Next.st_Att.Pitch	= f_Tgt_WrapAngle(st_State->st_Att.Pitch + (st_Rate->Pitch * stepTime));
	st_Next.st_Att.Yaw		= f_Tgt_WrapAngle(st_State->st_Att.Yaw + (st_Rate->Yaw * stepTime));
	...
	if ((f_Tgt_IsFiniteLla(&st_LlaMid) != 0) && (f_Tgt_IsFiniteLla(&st_Next.st_Lla) != 0) &&
		(f_Tgt_IsFiniteRect(&st_Next.st_PosEcef) != 0) && (f_Tgt_IsFiniteRect(&st_Next.st_VelEcef) != 0))
	{
...
// NED(기준점 기준 수평면) → 위경고도
EN_TgtStatus f_Tgt_NedToLla(STRUCT_Coord_Lla *st_Out, const STRUCT_Coord_Rect *st_Ned,
		const STRUCT_Coord_Rect *st_RefEcef, const STRUCT_Coord_Lla *st_RefLla)
{
	EN_TgtStatus		enStatus = TGT_ERR_NULL;
	STRUCT_Coord_Rect	st_Ecef;

	if ((st_Out != NULL) && (st_Ned != NULL) && (st_RefEcef != NULL) && (st_RefLla != NULL))
	{
		st_Ecef		= f_Trans_Ned_To_Ecef(st_Ned->x, st_Ned->y, st_Ned->z,
				st_RefEcef->x, st_RefEcef->y, st_RefEcef->z, st_RefLla->Lat, st_RefLla->Lon);
		*st_Out		= f_Trans_Ecef_To_Lla(st_Ecef.x, st_Ecef.y, st_Ecef.z);
		enStatus	= (f_Tgt_IsFiniteLla(st_Out) != 0) ? TGT_OK : TGT_ERR_COORD;
	}
...
EN_TgtStatus f_Tgt_LlaToNed(STRUCT_Coord_Rect *st_Out, const STRUCT_Coord_Lla *st_Lla,
		const STRUCT_Coord_Rect *st_RefEcef, const STRUCT_Coord_Lla *st_RefLla)
{
	EN_TgtStatus		enStatus = TGT_ERR_NULL;
	STRUCT_Coord_Rect	st_Ecef;
	...
	if (st_Lla != NULL)
	{
		...
		enStatus	= f_Tgt_EcefToNed(st_Out, &st_Ecef, st_RefEcef, st_RefLla);
	}
...
EN_TgtStatus f_Tgt_EcefToNed(STRUCT_Coord_Rect *st_Out, const STRUCT_Coord_Rect *st_Ecef,
		const STRUCT_Coord_Rect *st_RefEcef, const STRUCT_Coord_Lla *st_RefLla)
{
	EN_TgtStatus enStatus = TGT_ERR_NULL;

	if ((st_Out != NULL) && (st_Ecef != NULL) && (st_RefEcef != NULL) && (st_RefLla != NULL))
	{
		*st_Out		= f_Trans_Ecef_To_Ned(st_Ecef->x, st_Ecef->y, st_Ecef->z,
				st_RefEcef->x, st_RefEcef->y, st_RefEcef->z, st_RefLla->Lat, st_RefLla->Lon);
		enStatus	= (f_Tgt_IsFiniteRect(st_Out) != 0) ? TGT_OK : TGT_ERR_COORD;
	}

	return enStatus;
}

// ECEF 벡터를 NED 방향으로만 돌린다. 기준점을 원점으로 주어 자리 이동을 없앤다.
EN_TgtStatus f_Tgt_EcefVecToNed(STRUCT_Coord_Rect *st_Out, const STRUCT_Coord_Rect *st_Vec,
		const STRUCT_Coord_Lla *st_RefLla)
{
	const STRUCT_Coord_Rect st_Zero = { 0.0, 0.0, 0.0 };

	return f_Tgt_EcefToNed(st_Out, st_Vec, &st_Zero, st_RefLla);
}
