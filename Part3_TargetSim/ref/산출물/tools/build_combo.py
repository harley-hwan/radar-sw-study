...
글꼴은 맑은 고딕이 있으면 그걸 쓰고, 없으면(리눅스) 나눔고딕을 쓴다. make_figures.py 와 같은 규칙이다.
"""
import io
import os
import re
...
SLIDES_PDF = os.path.join(OUT_DIR, "Chapter3_TargetSim.pdf")
...
for REG, BOLD in FONT_CANDIDATES:
    ...
    sys.exit("한글 글꼴을 못 찾음 (맑은 고딕 / 나눔고딕 / 나눔바른고딕)")
FONT = {"mg": fitz.Font(fontfile=REG), "mgb": fitz.Font(fontfile=BOLD)}

DARK = (0.118, 0.137, 0.180)        # 머리글 · 쪽번호 배지
BODY = (0.239, 0.267, 0.318)        # 대본 본문
GOLD = (0.541, 0.369, 0.063)        # 구간 이름과 예상 시간
GREY = (0.478, 0.510, 0.561)        # 꼬리글
PANEL = (0.949, 0.957, 0.969)       # 대본 판

# 판형 (A4). 원본 합본과 같은 자리에 놓기 위한 값이라 손대지 않는다
PAGE_W, PAGE_H = 595.0, 842.0
BADGE = fitz.Rect(40, 34, 84, 59)
HEAD_Y = 51.0
SLIDE_BOX = fitz.Rect(40, 70.4, 556, 360.6)         # 16:9
PANEL_BOX = fitz.Rect(40, 381, 556, 797)
LABEL_Y = 400.0                                     # "발표 대본" 과 구간 이름의 글줄 바닥
BODY_X, BODY_W = 58.0, 479.0
BODY_Y0 = 420.0                                     # 본문 첫 줄 바닥
LEAD, PARA_GAP = 14.5, 22.5                         # 줄 간격, 문단 사이
FOOT_Y = 812.0


# 나눔고딕 · 맑은 고딕에 없는 글자. 그대로 넣으면 네모(.notdef)가 찍히므로 바꿔 준다
SUBST = {"\u2212": "-"}                              # MINUS SIGN -> HYPHEN-MINUS
...
# 숫자·영문이 이어지는 자리에서는 끊지 않는다 (1799.99996 이 1 / 799.99996 으로 갈라지지 않게)
def _glued(a, b):
    ok = "0123456789.,_-"
    return ((a.isascii() and (a.isalnum() or a in ok)) and
            (b.isascii() and (b.isalnum() or b in ok)))


def wrap(text, font, size, width):
    """글자 단위로 끊는다. 한글은 이렇게 끊어야 오른쪽 끝이 고르게 맞는다"""
    lines, cur = [], ""

    for ch in text:
        if cur and font.text_length(cur + ch, size) > width:
            # cur 와 ch 사이가 영문·숫자 토막 한가운데면 토막 앞까지 되돌아간다
            cut = len(cur)
            nxt = ch
            while cut > 1 and _glued(cur[cut - 1], nxt):
                nxt = cur[cut - 1]
                cut -= 1

            # 되돌릴 거리가 너무 길면(줄 대부분이 한 토막) 그냥 끊는다
            if cut <= 1 or (len(cur) - cut) > 24:
                cut = len(cur)

            lines.append(cur[:cut].rstrip())
            rest = cur[cut:]
            cur = rest + ("" if (ch == " " and not rest) else ch)
        else:
            cur += ch
...
def build(slides_pdf, md_path, out_pdf):
    ...
    for i in range(total):
        ...
        pg.insert_text(fitz.Point(96, HEAD_Y), title, fontname="mgb", fontsize=12,
                       color=DARK)
        pg.show_pdf_page(SLIDE_BOX, src, i)

        pg.insert_text(fitz.Point(BODY_X, LABEL_Y), "발표 대본", fontname="mgb",
        ...
        for para in body:
            for line in wrap(para, FONT["mg"], 10, BODY_W):
                pg.insert_text(fitz.Point(BODY_X, y), line, fontname="mg", fontsize=10,
                               color=BODY)
                y += LEAD
            y += PARA_GAP - LEAD
    ...
    doc.set_metadata({"title": "%s · 대본 합본" % TITLE, "author": AUTHOR,
                      "producer": "build_combo.py"})
    doc.subset_fonts()
