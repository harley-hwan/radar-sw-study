...
for i, (n, t, d) in enumerate(_idx):
    ...
    txt(s, 6.40, y + 0.10, 2.90, 0.40, t, size=15, color=NAVY, bold=True)
    txt(s, 9.45, y - 0.05, 3.20, 1.10, d, size=10, color=SLATE, space=1.42)
    if i < 2:
        hline(s, 5.25, y + 1.26, 7.35, PALE)
txt(s, 8.37, 6.98, 1.20, 0.30, str(_page), size=9, color=MUTED, align=PP_ALIGN.CENTER,
    font="Arial")

s, y = content(CH0, "과제 요구사항과 발표 구성",
               "「과제 3 내용」의 요구사항을 네 덩어리로 묶고, 각각을 어느 장에서 다루는지 먼저 짚겠습니다.")
_req = [("1  표적 구조체 정의",
         "표적 최대 10, 표적별 기동 최대 30.\n위치는 위도 · 경도 · 고도,\n속도는 동체 속도 V_heading 한 개.\n기동은 G · 축 · 시작 · 종료.", "01 장", BLUE),
        ("2  표적 상태 갱신",
         "위치와 속도를 ECEF 에서 갱신.\n추가 고민 — V_heading 을 ECEF\nVx, Vy, Vz 로 변환하는 방법.", "02 장", AMBER),
        ("3  시뮬레이션 조건",
...
banner(s, MX, y + 3.12, MW, 1.02,
       "좌표변환과 행렬 연산은 제공받은 코드를 한 줄도 고치지 않고 그대로 사용했습니다. 새로 작성한 것은 TargetSim.c 하나입니다.\n"
       "발표에 나오는 숫자는 모두 그 Core 를 실제로 빌드해 돌린 결과이며, 원본 CSV 를 함께 제출합니다.")

s, y = content(CH0, "표적 모의가 들어가는 자리",
               "레이다 데이터처리 소프트웨어를 만들려면, 그 앞단이 내보내는 것과 같은 규격의 입력이 필요합니다.")
picture(s, "fig01_why.png", body_box(y))

s, y = content(CH0, "소프트웨어 구성과 인터페이스",
               "연산과 화면을 분리해, 계산 엔진만 따로 빌드해 검증할 수 있게 했습니다.")
picture(s, "fig02_system.png", body_box(y))

# ═══════════════════════════════════════════════════════════════════════
# 01 장
# ═══════════════════════════════════════════════════════════════════════
section("01", "표적 구조체 설계", "Target State and Structure Design",
        ["표적 하나의 상태를 어떤 변수로 잡을 것인가",
         "\"기동\" 을 코드가 다룰 수 있는 네 개의 값으로 환원하기",
         "설정(입력)과 상태(출력)를 분리한 이유"])

s, y = content(CH1, "표적 상태 벡터의 정의",
               "위치 3 + 속도 3 + 자세각 3, 모두 아홉 개 성분으로 표적 하나의 상태가 결정됩니다.")
picture(s, "fig03_state.png", body_box(y))

s, y = content(CH1, "쓰는 좌표계 — ECEF · LLA · NED · Body",
               "Part 2 에서 정리한 좌표계 중 표적 모의에 실제로 필요한 것은 이 넷입니다.")
picture(s, "fig04_frames.png", body_box(y))

s, y = content(CH1, "자세각 정의와 회전 순서",
               "Roll · Pitch · Yaw 가 각각 어느 축 둘레의 회전인지, 그리고 곱하는 순서를 고정합니다.")
picture(s, "fig05_euler.png", body_box(y))

s, y = content(CH1, "기동을 네 개의 값으로",
               "\"12 초부터 16 초까지 오른쪽으로 8 G\" 라는 지시를 구조체 한 줄로 옮깁니다.")
picture(s, "fig06_maneuver.png", body_box(y))

s, y = content(CH1, "G 값이 각속도와 선회 반경을 정한다",
               "각속도를 직접 받지 않고 G 로 받는 이유는, G 가 조종사와 기체가 실제로 견디는 한계이기 때문입니다.")
...
code(s, MX, y + 0.02, 7.30, 4.72, [
    ...
    "} EN_TurnType;",
    "",
    ("// 기동 1건. gravityValue 의 부호가 회전 방향이며", "c"),
    ("// 동체축 오른손 법칙을 따른다.", "c"),
    ("typedef struct", "k"),
    "{",
    ...
    "} ST_TargetManeuver;",
    "",
    ("// 위치는 위도/경도/고도, 속도는 동체 x 축 방향 크기.", "c"),
    ("typedef struct", "k"),
    "{",
    ...
    "} ST_TargetInit;",
], size=8.8, lead=1.12, title="TargetSimCore/include/TargetSim.h")
card(s, 8.35, y + 0.02, 4.28, 1.50, "구간은 [시작, 종료)",
     "한 기동의 끝과 다음 기동의 시작이 같아도\n겹침으로 보지 않는다. 경계를 열어 두어야\n연속 기동을 한 줄씩 이어 붙일 수 있다.",
     accent=BLUE, bsize=9.6)
card(s, 8.35, y + 1.66, 4.28, 1.50, "겹치면 계산 전에 거부",
     "같은 표적에서 두 구간이 실제로 겹치면\n어느 쪽을 따를지 정의되지 않는다.\nTGT_ERR_MANEUVER_OVERLAP 을 돌려준다.",
     accent=AMBER, bsize=9.6)
card(s, 8.35, y + 3.30, 4.28, 1.44, "10 × 30 은 고정 배열",
     "구조체 하나가 약 10 KB. 동적 할당이 없어\n실행 중 메모리 실패가 생길 자리가 없다.",
     accent=OK, bsize=9.6)
...
card(s, 8.35, y + 0.02, 4.28, 2.28, "ECEF 와 LLA 를 함께 두는 이유",
     "· 갱신은 ECEF 에서 해야 곡률 항이 사라진다\n"
     "· 출력 · 화면 · CSV 는 LLA 라야 읽힌다\n"
     "· 다음 스텝의 속도를 만들 때도 그 지점의\n   위도 · 경도가 필요하다\n"
     "스텝당 한 번만 변환해 두면 세 용도를 모두 덮는다.",
     accent=BLUE, bsize=9.4)
card(s, 8.35, y + 2.44, 4.28, 2.30, "플랫폼도 같은 구조체",
     "플랫폼은 기동이 없는 표적일 뿐이다.\n같은 ST_TargetState 를 쓰고 각속도만 0 으로\n"
     "넣어 같은 적분 경로를 통과시킨다.\n\n"
     "속력으로 나누는 연산은 기동 조회 안에만 있어,\n정지 플랫폼(V = 0)도 안전하다.",
     accent=OK, bsize=9.4)
...
section("02", "표적 궤적 모의 구현", "Trajectory Propagation",
        ["표적 운동을 지배하는 방정식과 그 가정",
         "동체 속도 한 개를 ECEF 속도 세 성분으로 — 과제의 추가 고민 항목",
         "0.1 초를 전진시키는 방법과 오차 차수"])

s, y = content(CH2, "표적 운동을 지배하는 세 줄",
               "이 세 줄이 곧 구현이고, 아래 세 가지 가정을 명시적으로 두고 있습니다.")
picture(s, "fig10_model.png", body_box(y))

s, y = content(CH2, "왜 ECEF 에서 적분하는가",
               "명세가 정한 조건이지만, 왜 그래야 하는지는 오차 구조를 보면 분명합니다.")
...
s, y = content(CH2, "회전행렬의 원소",
               "두 행렬을 펼치면 이렇게 됩니다. 위치에는 평행이동이 붙고, 속도에는 붙지 않습니다.")
picture(s, "fig13_dcm.png", body_box(y))
...
s, y = content(CH2, "0.1 초를 어떻게 전진시킬 것인가",
               "속도가 계속 바뀌는 구간에서 어느 지점의 기울기를 쓰느냐가 오차 차수를 가릅니다.")
...
card(s, 8.52, y + 0.05, 4.11, 2.28, "② 를 빼먹었을 때 생긴 일",
     "처음에는 자세만 반 스텝 돌리고 위치는 그대로\n두었다. 그러면 스텝 시작 지점의 위경도로\n"
     "NED 를 잡게 되어, 표적은 앞으로 나아갔는데\n\"북쪽\" 의 방향은 뒤에 남는다.\n\n"
     "그 차이가 매 스텝 d²/2R 만큼 고도로 쌓인다.",
     accent=RED, bsize=9.2)
card(s, 8.52, y + 2.47, 4.11, 2.28, "알고도 남겨 둔 한계 — 짐벌락",
     "오일러각을 직접 적분하므로 Pitch 가 ±90°\n근처면 Yaw 축이 속도 방향과 겹쳐 Yaw 기동이\n"
     "궤적을 바꾸지 못한다.\n\n사원수로 바꾸면 풀리지만 과제 범위를 넘는다고\n보고 코드에 주석으로 명시했다.",
     accent=AMBER, bsize=9.2)

s, y = content(CH2, "한 스텝 알고리즘과 계산 비용")
picture(s, "fig15_algorithm.png", body_box(y, 6.80))

s, y = content(CH2, "계산이 깨져도 상태는 지킨다",
               "제공받은 변환 함수는 실패를 알려 주지 않습니다. 그래서 결과의 유한성을 직접 확인합니다.")
picture(s, "fig16_guard.png", body_box(y))

# ═══════════════════════════════════════════════════════════════════════
# 03 장
# ═══════════════════════════════════════════════════════════════════════
section("03", "시뮬레이션 결과 및 검증", "Results and Verification",
        ["명세 조건 그대로 60 초를 돌린 결과",
         "해석해 대조 · 불변량 보존 · 수렴 차수 — 세 가지 검증",
         "기동을 얹은 시연과 운용 화면"])
...
card(s, MX, end + 0.30, 3.85, 1.42, "시간 60 s · 간격 0.1 s",
     "스텝 600 회, 표본 601 개.\nt = 0 과 t = 60 을 모두 포함한다.\n"
     "시각은 누적이 아니라 스텝 번호 × 간격으로\n매겨 반올림 오차가 쌓이지 않게 했다.",
     accent=BLUE, bsize=9.4)
...
card(s, MX + 8.10, end + 0.30, 3.83, 1.42, "플랫폼 속력이 0 인 점",
     ...
     "플랫폼은 기동이 없어 그 자리를 지나가지 않는다.",
     accent=AMBER, bsize=9.4)

s, y = content(CH3, "결과 ① — 60 초 궤적",
               "플랫폼을 원점으로 잡은 동-북 평면입니다. 점 하나가 10 초입니다.")
picture(s, "fig17_result_map.png", body_box(y))

s, y = content(CH3, "결과 ② — 불변량 잔차",
               "변하면 안 되는 값이 실제로 얼마나 변했는지, 마이크로미터 단위까지 봤습니다.")
picture(s, "fig18_invariant.png", body_box(y))

s, y = content(CH3, "결과 ③ — 검증 종합")
picture(s, "fig19_verify.png", body_box(y, 6.80))

s, y = content(CH3, "결과 ④ — 갱신 간격에 대한 수렴",
               "값 하나가 작은 것보다, 간격을 바꿨을 때 정해진 차수로 줄어드는 것이 더 강한 증거입니다.")
picture(s, "fig20_convergence.png", body_box(y))

s, y = content(CH3, "결과 ⑤ — 기동을 얹으면",
               "초기값 · 시뮬레이션 시간 · 갱신 간격은 명세 그대로, 기동표 10줄만 추가했습니다.")
picture(s, "fig21_zigzag.png", body_box(y))

s, y = content(CH3, "TargetSim — 값을 바꿔 가며 확인하는 화면",
               "표에서 숫자를 고치면 자동으로 다시 돌고, 그림에서 시작점과 기수를 끌 수 있습니다.")
picture(s, "fig22_ui.png", (MX, y - 0.05, 7.55, 4.52))
card(s, 8.55, y + 0.00, 4.08, 1.40, "고치면 곧바로 다시 돈다",
     "표의 칸을 고치면 0.25 초 뒤 자동으로 다시\n계산한다. 601 표본 × 3 객체가 4.8 ms 라\n기다릴 일이 없다.", accent=BLUE, bsize=9.6)
card(s, 8.55, y + 1.55, 4.08, 1.40, "그림에서 끌어서 배치",
     "시작점과 기수 손잡이를 마우스로 끌면\n위도 · 경도 · Yaw 가 표에 반영된다.\n"
     "회전각과 선회 반경은 G 로부터 계산해 보여 준다.", accent=AMBER, bsize=9.6)
card(s, 8.55, y + 3.10, 4.08, 1.42, "결과는 표 · CSV · 그림",
     "시각별 LLA 를 표로 보고, 슬라이더로 되감고,\nCSV 로 내보낸다. 시나리오는 텍스트 파일로\n저장해 다시 불러올 수 있다.", accent=OK, bsize=9.6)
